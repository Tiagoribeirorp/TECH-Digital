...
def roll_initiative(
    ...
    """Roll initiative once at combat start.

    The source rule is Tempo de Reação + 1d20. Tempo de Reação is based on
    Percepção; the d20 is injected so tests and future deterministic replays
    can control the random result.
    """
    ...
    for combatant in combat.participants:
        ...
        combatant.initiative_result = (
            combatant.character.derive_stats().reaction_time_base + roll
        )

    ordered = sorted(
        combat.participants,
        key=lambda item: item.initiative_result if item.initiative_result is not None else -1,
        reverse=True,
    )
    ...
    combat.combat_log.append(
        {
            ...
            "order": list(combat.initiative_order),
        }
    )
...
def declaration_order(combat: CombatState) -> list[str]:
    """Return declaration order: slower participants declare first."""
...
def execution_order(combat: CombatState) -> list[str]:
    """Return execution order: faster participants execute first."""
    if not combat.initiative_order:
...
def resolve_basic_attack(
    ...
    *,
    ...
    """Apply a basic attack result using already-resolved rule values.

    This function intentionally does not define an attack formula, defense
    formula, weapon damage formula, critical damage rule, or armor interaction.
    Those remain data/rules inputs until the source material validates them.
    """
...
def finish_round(combat: CombatState) -> None:
    ...
    if len(active) <= 1:
        ...
        combat.combat_log.append(
            {"event": "combat_finished", "round": combat.round_number}
        )
        return
    ...
    combat.phase = CombatPhase.DECLARATION
    combat.combat_log.append(
        {"event": "round_started", "round": combat.round_number}
    )
